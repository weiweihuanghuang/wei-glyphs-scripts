#MenuTitle: Copy Layer to Every Master Layer
# -*- coding: utf-8 -*-
__doc__="""
Copies one master to every master layer in selected glyphs.
"""
...
class MasterFiller( object ):
	...
	def GetMasterNames( self, option ):
		myMasterList = []
	...
	def buttonCallback( self, sender ):
		...
		masterIndexList = self.GetMasterNames(2)
		...
		for thisGlyph in selectedGlyphs:
			try:
				for targetlayerIndex in [x for i, x in enumerate(masterIndexList) if x != index_from]:
					print "\nProcessing", thisGlyph.name
					sourcelayer = thisGlyph.layers[ index_from ]
					targetlayer = thisGlyph.layers[ targetlayerIndex ]
